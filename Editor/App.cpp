    ...
    void App::Frame(float deltaTime)
    {
      ...
      auto pbrTestFn = [this]()
      {
        ...
        if (test)
        {
          ...
          EntityRawPtrArray a = GetCurrentScene()->GetEntities();
          for (Entity* n : a)
          {
            GetCurrentScene()->RemoveEntity(n->GetIdVal());
          }

          EngineSettings::GraphicSettings& gfx =
              Main::GetInstance()->m_engineSettings.Graphics;

          gfx.bloomIntensity = 0.0f;
          gfx.bloomThreshold = 1000.0f;
          gfx.TonemapperMode = TonemapMethod::Reinhard;

          float lDist        = 10.0f;
          float num          = 7.0f;
          float step         = 1.0f / (num - 1.0f);
          float trStart      = 1.0f;
          float trStep       = 2.0f;
          for (float i = 0.0f; i < num; i += 1.0f)
          {
            for (float j = 0.0; j < num; j += 1.0f)
            {
              Sphere* nt = new Sphere(1.0f);
              nt->m_node->SetTranslation(
                  Vec3(trStart + trStep * i, trStart + trStep * j, 0.0f));

              MaterialPtr mat     = std::make_shared<Material>();
              mat->m_materialType = MaterialType::PBR;
              mat->GetRenderState()->useForwardPath  = true;
              mat->GetRenderState()->isColorMaterial = true;
              mat->m_color                           = Vec3(0.5f, 0.0f, 0.0f);
              mat->m_metallic  = std::clamp(j * step, 0.0f, 1.0f);
              mat->m_roughness = std::clamp(i * step, 0.05f, 1.0f);
              mat->Init();
              MaterialComponent* mc = new MaterialComponent();
              mc->SetMaterialVal(mat);
              nt->AddComponent(mc);
              GetCurrentScene()->AddEntity(nt);
            }
          }

          float center    = trStart + trStep * (num - 1.0f) / 2.0f;
          float lStep     = 8.0f;
          Vec3Array poses = {Vec3(center - lStep, center - lStep, lDist),
                             Vec3(center - lStep, center + lStep, lDist),
                             Vec3(center + lStep, center - lStep, lDist),
                             Vec3(center + lStep, center + lStep, lDist)};
          ...
          {
            EditorPointLight* pl = new EditorPointLight();
            pl->m_node->SetTranslation(poses[i]);
            pl->Init();
            pl->SetRadiusVal(lDist * 10.0f);
            pl->SetIntensityVal(7.5f);
            GetCurrentScene()->AddEntity(pl);
          }
        }
      };
      pbrTestFn();
