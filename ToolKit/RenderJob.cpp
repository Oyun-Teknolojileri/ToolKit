...
namespace ToolKit
{
                                            ...
                                            const EnvironmentComponentPtrArray& environments)
  {
             ...
             [&](Entity* ntt) -> bool
             {
               if (ntt->IsVisible() || ignoreVisibility)
               {
                 if (MeshComponent* meshComp = ntt->GetComponentFast<MeshComponent>())
                 {
                   ...
                   int meshCount = meshComp->GetMeshVal()->GetMeshCount();
                   if (meshCount == 0)
                   {
                     return true;
                   }
    ...
    jobArray.clear();
    jobArray.reserve(size); // at least.
  ...
  void RenderJobProcessor::ForceCreateRenderJobs(RenderJobArray& jobArray, Entity* entity)
  {
    ...
    // Reserve for extra jobs.
    jobArray.reserve(jobArray.size() + allMeshes.size());
    ...
    for (int subMeshIndx = 0; subMeshIndx < (int) allMeshes.size(); subMeshIndx++)
    {
      ...
      jobArray.emplace_back(std::move(job));
    }
  }
