  ...
  class TK_API FileManager
  {
   ...
   private:
    ...
    XmlFilePtr ReadXmlFileFromZip(zipFile zfile,
                                  ...
                                  const char* path);
    uint8* ReadImageFileFromZip(zipFile zfile,
                                const String& relativePath,
                                ImageFileInfo& fileInfo);
    float* ReadHdriFileFromZip(zipFile zfile,
                               const String& relativePath,
                               ImageFileInfo& fileInfo);
    XmlFilePtr CreateXmlFileFromZip(zipFile zfile,
                                    const String& filename,
                                    uint filesize);
    uint8* CreateImageFileFromZip(zipFile zfile,
                                  uint filesize,
                                  ImageFileInfo& fileInfo);
    float* CreateHdriFileFromZip(zipFile zfile,
                                 uint filesize,
                                 ImageFileInfo& fileInfo);
