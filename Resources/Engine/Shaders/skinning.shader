<shader>
  ...
  // keyFramesData-> x: keyFrame1 y: keyFrame2 z: time w: key frame count
  void skinCalc(in sampler2D animDataTexture, vec4 keyFramesData, in vec4 vertexPos, in vec3 vertexNormal, out vec4 skinnedPos, out vec3 skinnedNormal)
  {
    skinnedPos      = vec4(0.0);
  ...
  // keyFramesData-> x: keyFrame1 y: keyFrame2 z: time w: key frame count
  void skinCalc(in sampler2D animDataTexture, vec4 keyFramesData, in vec4 vertexPos, in vec3 vertexNormal, in vec3 vertexBiTangent,
  ...
  void skin(in vec4 vertexPos, out vec4 skinnedPos)
  {
    ...
    if (blendAnimation != 0)
    {
      ...
      skinCalc(s_texture2, vec4(blendKeyFrame1, blendKeyFrame2, blendKeyFrameIntepolationTime, blendKeyFrameCount), vertexPos, blendingAnimSkinnedPos);
  ...
  {
    skinCalc(s_texture3, vec4(keyFrame1, keyFrame2, keyFrameIntepolationTime, keyFrameCount), vertexPos, vertexNormal, skinnedPos, skinnedNormal);
  ...
  void skin(in vec4 vertexPos, in vec3 vertexNormal, in vec3 vertexBiTangent,
    out vec4 skinnedPos, out vec3 skinnedNormal, out vec3 skinnedBiTangent)
  {
    skinCalc(s_texture3, vec4(keyFrame1, keyFrame2, keyFrameIntepolationTime, keyFrameCount), vertexPos, vertexNormal, vertexBiTangent,
