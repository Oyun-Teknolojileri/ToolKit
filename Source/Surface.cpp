...
ToolKit::Surface::Surface(std::shared_ptr<Texture> texture, const std::vector<Vertex>& vertices)
{
  m_mesh->m_material->m_diffuseTexture = texture;
...
ToolKit::Surface::Surface(std::string file, glm::vec2 pivotOffset)
{
  m_file = file;
  m_pivotOffset = pivotOffset;
}
...
void ToolKit::Surface::Init(bool flushClientSideArray)
{
  if (m_initiated)
    return;
